...
/**
 * Set SharePoint context for data operations
 * @param {Object} context - SharePoint context with siteUrl and requestDigest
 */
export function setDataServiceContext(context) {
...
export function getSharePointContext() {
    // If context is not set, try to get it from the global config
    if (!sharePointContext.siteUrl && window.CONFIG_GLOBAL) {
        sharePointContext.siteUrl = window.CONFIG_GLOBAL.SITE_URL || window.location.origin;
    }
 ...
 */
export async function getListItems(listName, selectFields = '*', filterQuery = '', orderBy = 'Id', top = 1000) {
    try {
        const context = getSharePointContext();
        if (!context.siteUrl) {
            throw new Error('SharePoint context niet geïnitialiseerd');
        }
...
export async function createListItem(listName, itemData) {
    try {
        ...
        if (!context.siteUrl || !context.requestDigest) {
            throw new Error('SharePoint context niet volledig geïnitialiseerd');
        }
 ...
 */
export async function getChoiceFieldOptions(listName, fieldName) {
    try {
        const context = getSharePointContext();
        if (!context.siteUrl) {
            throw new Error('SharePoint context niet geïnitialiseerd');
