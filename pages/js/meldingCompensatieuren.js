document.addEventListener('DOMContentLoaded', function() {
            ...
            function setDefaultDateTimes() {
                ...
                const huidigeTijd = nu.toTimeString().slice(0,5); // HH:mm

                // Set start date to today (or selected date if available)
                startCompensatieDatumInput.value = vandaagISO;
                startCompensatieTijdInput.value = huidigeUur < 12 ? '09:00' : huidigeTijd;

                // Always set end date to today for compensatie
                eindeCompensatieDatumInput.value = vandaagISO;
                eindeCompensatieTijdInput.value = huidigeUur < 17 ? '17:00' : huidigeTijd;
                
                console.log("[setDefaultDateTimes] Set default dates - Start:", startCompensatieDatumInput.value, "End:", eindeCompensatieDatumInput.value);
                
                // Vul ook de hidden ISO velden in
                synchroniseerCompensatieDatumVelden();
                
                berekenUrenTotaal(); // Herbereken uren
            }

            /**
 * Synchroniseert de hidden ISO datum velden voor compensatie uren
 * Deze functie moet aangeroepen worden wanneer de zichtbare datumvelden worden gewijzigd
 */
function synchroniseerCompensatieDatumVelden() {
    const startDatumInput = document.getElementById('StartCompensatieDatum');
    const startTijdInput = document.getElementById('StartCompensatieTijd');
    const eindDatumInput = document.getElementById('EindeCompensatieDatum');
    const eindTijdInput = document.getElementById('EindeCompensatieTijd');
    
    const hiddenStartISO = document.getElementById('StartCompensatieUrenISO');
    const hiddenEindISO = document.getElementById('EindeCompensatieUrenISO');
    
    console.log('[CompensatieUren] Synchronizing hidden ISO date fields...');
    
    if (startDatumInput && startTijdInput && hiddenStartISO) {
        if (startDatumInput.value && startTijdInput.value) {
            const startDateTime = new Date(`${startDatumInput.value}T${startTijdInput.value}`);
            hiddenStartISO.value = startDateTime.toISOString();
            console.log('[CompensatieUren] ✓ StartCompensatieUrenISO synchronized:', hiddenStartISO.value);
        }
    }
    
    if (eindDatumInput && eindTijdInput && hiddenEindISO) {
        if (eindDatumInput.value && eindTijdInput.value) {
            const eindDateTime = new Date(`${eindDatumInput.value}T${eindTijdInput.value}`);
            hiddenEindISO.value = eindDateTime.toISOString();
            console.log('[CompensatieUren] ✓ EindeCompensatieUrenISO synchronized:', hiddenEindISO.value);
        }
    }
    
    // Bereken ook het totaal uren als beschikbaar
    if (typeof berekenUrenTotaal === 'function') {
        berekenUrenTotaal();
    }
}

// Export synchronization function to global scope for debugging
window.synchroniseerCompensatieDatumVelden = synchroniseerCompensatieDatumVelden;
            ...
            [startCompensatieDatumInput, startCompensatieTijdInput, eindeCompensatieDatumInput, eindeCompensatieTijdInput].forEach(input => {
                if (input) { // Check of element bestaat
                    input.addEventListener('change', () => {
                        berekenUrenTotaal();
                        synchroniseerCompensatieDatumVelden(); // Synchroniseer verborgen velden
                    });
                }
            });
...
// Export functions globally for modal compatibility
window.initializeCompensatieUrenFormulierLogica = function(medewerkerContext, selectedDate) {
    console.log("[CompensatieModal] Initializing with context:", medewerkerContext, "and date:", selectedDate);
    
    // Set the employee fields
    if (medewerkerContext) {
        const medewerkerDisplayInput = document.getElementById('MedewerkerDisplay');
        const medewerkerIdInput = document.getElementById('MedewerkerID');
        const medewerkerIdDisplayInput = document.getElementById('MedewerkerIDDisplay');
        
        if (medewerkerDisplayInput) {
            medewerkerDisplayInput.value = medewerkerContext.Naam || medewerkerContext.Title || medewerkerContext.displayName || '';
            console.log("[CompensatieModal] Set MedewerkerDisplay:", medewerkerDisplayInput.value);
        }
        if (medewerkerIdInput) {
            medewerkerIdInput.value = medewerkerContext.normalizedUsername || medewerkerContext.Username || medewerkerContext.loginNaam || '';
            console.log("[CompensatieModal] Set MedewerkerID:", medewerkerIdInput.value);
        }
        if (medewerkerIdDisplayInput) {
            medewerkerIdDisplayInput.value = medewerkerContext.normalizedUsername || medewerkerContext.Username || medewerkerContext.loginNaam || '';
            console.log("[CompensatieModal] Set MedewerkerIDDisplay:", medewerkerIdDisplayInput.value);
        }
    }
    
    // Set the date fields
    const today = new Date();
    const startDateInput = document.getElementById('StartCompensatieDatum');
    const endDateInput = document.getElementById('EindeCompensatieDatum');
    
    if (startDateInput) {
        // Use selected date if provided, otherwise use today
        const useDate = selectedDate || today;
        startDateInput.value = useDate.toISOString().split('T')[0];
        console.log("[CompensatieModal] Set StartCompensatieDatum:", startDateInput.value);
    }
    
    // Always set end date to today for compensatie
    if (endDateInput) {
        endDateInput.value = today.toISOString().split('T')[0];
        console.log("[CompensatieModal] Set EindeCompensatieDatum to today:", endDateInput.value);
    }
    
    // Set default times
    const startTimeInput = document.getElementById('StartCompensatieTijd');
    const endTimeInput = document.getElementById('EindeCompensatieTijd');
    
    if (startTimeInput && !startTimeInput.value) {
        startTimeInput.value = '09:00';
        console.log("[CompensatieModal] Set default start time: 09:00");
    }
    if (endTimeInput && !endTimeInput.value) {
        endTimeInput.value = '17:00';
        console.log("[CompensatieModal] Set default end time: 17:00");
    }
    
    // Synchronize hidden fields and calculate hours
    setTimeout(() => {
        if (typeof synchroniseerCompensatieDatumVelden === 'function') {
            synchroniseerCompensatieDatumVelden();
            console.log("[CompensatieModal] Synchronized hidden fields");
        }
        
        if (typeof berekenUrenTotaal === 'function') {
            berekenUrenTotaal();
            console.log("[CompensatieModal] Calculated total hours");
        }
    }, 100);
};

// Export the submit function for modal use
window.submitCompensatieAanvraag = async function() {
    console.log("[CompensatieModal] Submit function called");
    
    // Use the existing submit button click handler
    const submitButton = document.getElementById('submit-button');
    if (submitButton) {
        submitButton.click();
        return true;
    } else {
        console.error("[CompensatieModal] Submit button not found");
        return false;
    }
};